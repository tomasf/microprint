...
static NSString *const S3D_COMMENT = @" G-Code generated by Simplify3D";
static int const S3D_COMMENT_RANGE = 20;    // Simplify3d comment within this number of lines
static int const S3D_MAX_RANGE = 300;    // Simplify3d profile within this number of lines
static NSString *const PROFILE_REGEX = @"^\\s*(\\S+[\\S ]*?)\\s*=\\s*(\\S.*)$";
static NSString *const S3D_REGEX = @"^\\s{3}([A-Za-z][A-Za-z0-9]*),(.*)$";
...
- (instancetype)initFromLines: (NSArray<TFPGCode *> *)lines {
    if (self = [super init]) {
        ...
        } else if ([self hasS3dProfile: lines]) {
            self.profileType = S3dProfile;
            [self loadS3dProfile:lines];
    ...
    if (!retVal) {
        switch (self.profileType) {
            ...
            case S3dProfile:
                // Only check for the things we need to translate ...
                if ([key isEqualToString:@"Layer Height"]) {
                    retVal = self.values[@"layerHeight"];

                } else if ([key isEqualToString: @"Wall Thickness"]) {
                    retVal = @(self.values[@"perimeterOutlines"].doubleValue *
                    self.values[@"extruderWidth"].doubleValue).stringValue;

                } else if ([key isEqualToString:@"Fill Density"]) {
                    retVal = self.values[@"infillPercentage"];

                } else if ([key isEqualToString: @"Bed Adhesion"]) {
                    if ([self.values[@"useRaft"] isEqualToString:@"1"]) {
                        retVal = @"Raft";
                    } else {
                        retVal = @"None";
                    }
                    ...
                    retVal = self.values[@"generateSupport"];
                    
                } else if ([key isEqualToString: @"Print Speed"]) {
                    retVal = @(self.values[@"defaultSpeed"].doubleValue / 60.0).stringValue;
...
- (BOOL)hasS3dProfile:(NSArray<TFPGCode *> *)lines {
    BOOL hasOne = false;

    // Only check the first few lines
    for (int i = 0; i < lines.count && i < S3D_COMMENT_RANGE; i++) {
        if ([lines[i].comment hasPrefix:S3D_COMMENT]) {
            hasOne = true;
            break;
        }
    }
...
/*
 The S3d profile is embedded within the gcode as comment lines. The general format is "; key,value"
 */
- (BOOL)loadS3dProfile:(NSArray<TFPGCode *> *)lines {
    if(lines.count > 0) {
        BOOL inProfile = false;

        NSError *error = NULL;
        NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:S3D_REGEX options:0 error:&error];
        ...
        for (int i = 0; i < lines.count && i < S3D_MAX_RANGE; i++) {
            TFPGCode *thisLine = lines[i];
            if(!inProfile &&                    // We haven't found the start of the profile yet
                    ![thisLine hasFields] &&    // It's only a comment
                    thisLine.comment &&         // and actually has a comment
                    [thisLine.comment hasPrefix:S3D_COMMENT]) {   // Check to be see if it's an S3d profile
                inProfile = true;               // The we have the start of the profile
            }

            if (inProfile) {
                if(thisLine.comment){
                    NSArray<NSTextCheckingResult *> *matches = [regex matchesInString:thisLine.comment options:0 range:NSMakeRange(0, thisLine.comment.length)];

                    if(matches.count>0) {
                        NSString *key = [thisLine.comment substringWithRange:[matches[0] rangeAtIndex:1]];
                        NSString *val = [thisLine.comment substringWithRange:[matches[0] rangeAtIndex:2]];
                    ...
                    break;  // A non-comment exits the profile
                }
            }
        }
    }
